...
public abstract class Traversal {
    ...
    public void traverse(Collection<Integer> V0) {
        ...
        while (!_fringe.isEmpty()) {
            int pop = _fringe.remove();
            if (!marked(pop)) {
                mark(pop);
                _fringe.add(pop);
                Iteration<Integer> successors = _G.successors(pop);
                if (!visit(pop)) {
                    break;
                }
                reverse.clear();
                while (successors.hasNext()) {
                    int current = successors.next();
                    if (processSuccessor(pop, current)) {
                        reverse.add(current);
                    }
                }
                if (reverseSuccessors(pop)) {
                    Collections.reverse(reverse);
                }
                _fringe.addAll(reverse);
            } else if (shouldPostVisit(pop)
                && !posted.contains(pop)) {
                posted.add(pop);
                postVisit(pop);
            }
    ...
    /** Stores all vertexes that are marked. */
    protected ArrayList<Integer> _marked = new ArrayList<Integer>();
    /** Arraylist of those for reverse.*/
    protected ArrayList<Integer> reverse = new ArrayList<Integer>();
