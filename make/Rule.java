...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!commands.isEmpty() && !_commands.isEmpty()) {
            throw new IllegalStateException("They're not empty");
        } else {
    ...
    private void checkFinishedDependencies() {
        for (Integer success : _depends.successors(getVertex())) {
    ...
    private boolean outOfDate() {
        if (getTime() != null) {
            for (Integer success : _depends.successors(getVertex())) {
                if (_depends.getLabel(success).getTime() > getTime()) {
                    return true;
                }
            }
            return false;
